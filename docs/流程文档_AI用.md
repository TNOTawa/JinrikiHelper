...
- **MFA补对齐**: 为缺少 TextGrid 的音源包补充 MFA 对齐数据
- 使用临时工作空间，处理完成后自动清理
- 无需本地持久化存储
- **音频时长限制**: 单个音频文件最长 10 分钟，超时文件自动过滤

**MFA补对齐功能**:
- 上传包含 slices 目录（.wav + .lab 文件）的音源压缩包
- 自动检测音源包中的 TextGrid 覆盖率
- 选择语言后执行 MFA 强制对齐
- 生成 TextGrid 文件并更新 meta.json
- 打包完整音源包供下载

**MFA 不可用时的处理**:
- 制作音源时如果 MFA 不可用或对齐失败，会在日志中提示用户前往「MFA补对齐」页面修复
- MFA 验证超时从 30 秒增加到 120 秒，适应云端冷启动场景
- 超时但命令存在时，假设 MFA 可用并继续尝试对齐

**用户体验优化**:
- 音频未上传完成时禁用「开始制作」按钮，防止误操作
- 导出页面提供「使用刚制作的音源」按钮，避免重复上传
- Whisper 模型选项标注速度参考：small 约 4 秒/句，medium 约 12 秒/句（慢 2-3 倍但更准确）
- **处理时长估算**: 上传音频后自动显示预估处理时间（基于实测数据：每分钟音频约需 5.1 分钟处理）
- **导出插件动态选项系统**: 
